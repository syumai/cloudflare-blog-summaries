...
- 公開日: YYYY-MM-DD
- 関連: [[他記事への相対リンク]]
- GitHub: [docs/articles/YYYY-MM-DD-<slug>.md](https://github.com/syumai/cloudflare-blog-summaries/blob/main/docs/articles/YYYY-MM-DD-<slug>.md)
...
- 文体は平易な日本語にする。「通底」のような普段使わない硬い語は使わず、「共通する」「根底にある」などの自然な表現を選ぶ（Wiki・スライド・クイズ共通）。
...
- Worker をデプロイした場合は README に Demo セクションとして公開 URL を記載する（デプロイの判断はユーザー指示による）。
...
3. アジェンダ
4. 背景・課題（1〜3枚）
5. 本編: 発表内容／アーキテクチャ（図をURL直接参照で引用）
6. **コード例**（Slidevのコードハイライト機能・行フォーカス `{1-3|5|all}` などを活用。必須）
7. **ユースケース**（具体例を1ユースケース1枚程度。必須）
8. まとめ・所感
9. 参考リンク
...
- **横長の画像に `layout: image-right` / `image-left` を使わない**: 左右分割は横長画像が縮小されて見にくくなる。横長（目安: 幅が高さの1.3倍以上）の画像は、本文を短くまとめて画像を下に全幅で置く（`<img>` に `max-height` を指定して中央寄せ）か、テキストのスライドと画像のスライドにページを分ける。`image-right` は縦長・正方形に近い画像（スクリーンショット等）にのみ使い、必ず `backgroundSize: contain` を併記する。
- コード例とユースケースは必ず含める（記事にコード例が無い場合は、代表的なAPI利用イメージやアーキテクチャ図の読み解きで代替してもよいが、その旨を分かりやすく示す）。
- **コードと解説を分離しない**: 「コードだけのスライド→次のスライドで解説」という構成は、解説がコードのどこを指すのか分からなくなるため禁止。短いコード（目安20行以下）は同一スライドに解説を併置する（`layout: two-cols` でコード＋要点、またはコード直下に1〜3行の要点）。収まらない長いコードは、先に「何をするコードか・見どころ」を説明するスライドを置いてからコードを示し、解説では行番号や識別子名で該当箇所を明示する。
...
- **画像クリック拡大**: 全デッキ共通のライトボックス（`slides/theme/image-zoom.ts`）があり、スライド中の画像をクリックすると全画面拡大できる。新しいデッキを作る際は `slides/<slug>/setup/main.ts` を既存デッキからコピーして配置する（`defineAppSetup` で `setupImageZoom()` を呼ぶ3行のファイル）。
