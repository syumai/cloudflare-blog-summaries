...
- Cloudflareは、社内知識を活用したアプリ開発・業務自動化・社内システムへの安全なアクセスを実現する社内プラットフォーム「Cloudflare OS」をオープンソース公開
- 今年5月の全社展開以来、数千人が日常的に利用
- 構成要素は「エージェントワークスペース」「セキュリティ・ガバナンス基盤（Gatekeeper）」「個人向けアプリのプラットフォーム」の3つ
- エージェントは無権限からスタートし、管理者の許可を経て型付きバインディングとしてコードへ渡される。参照情報の共有・公開は**Gatekeeper**が制御
- コアリポジトリとstarterサンプルの2つをGitHubで公開し、自社アカウントで構築・カスタマイズ可能
